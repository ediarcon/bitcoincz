...
RUN apt-get update && apt-get install $APT_ARGS git zip wget unzip && rm -rf /var/lib/apt/lists/*
RUN apt-get update && apt-get install $APT_ARGS g++ && rm -rf /var/lib/apt/lists/*
RUN apt-get update && apt-get install $APT_ARGS bison autotools-dev libtool m4 automake autoconf pkg-config && rm -rf /var/lib/apt/lists/*
